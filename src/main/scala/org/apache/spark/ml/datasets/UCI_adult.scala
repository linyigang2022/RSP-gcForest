...
class UCI_adult extends BaseDatasets {
  ...
  def load_data(spark: SparkSession,
    ...
    val dataRDD = raw.rdd.filter(row => row.mkString.length > 1 && !row.mkString.startsWith("|"))
      ...
      require(splits.length == 15 || splits.length == 115, s"row $idx: $line has no 15 features, length: ${row.length}")
      val label = if (splits.length == 15) {
        if (splits(14).contains("<=50K")) 0.0d else 1.0d
      } else {
        splits(114).toDouble
      }
