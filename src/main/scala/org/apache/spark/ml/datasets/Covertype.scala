...
//[0 1 0]
//[0 0 1]
class Covertype extends BaseDatasets {
  /**
  ...
  def load_data(spark: SparkSession,
    ...
    val dataRDD = raw.rdd.filter(row => row.mkString.length > 1 && !row.mkString.startsWith("|"))
      ...
      //      logInfo(s"splits.length：${splits.length}")
      //      require(splits.length == 55, s"row $idx: $line has no 54 features, length: ${splits.length}")
      //      val label = splits(54).toDouble-1
            require(splits.length == 55 || splits.length == 62, s"row $idx: $line has no 54 or 61 features, length: ${splits.length}")
      val label = if (splits.length == 55) {
        splits(54).toDouble-1
      } else {
        splits(61).toDouble
      }
