...
//      logInfo(s"line:$line")
    ...
    val dataRDD = raw.rdd.filter(row => row.mkString.length > 1 && !row.mkString.startsWith("|"))
      ...
      require(splits.length == 6 || splits.length == 24, s"row $idx: $line has no 5 or 23 features, length: ${splits.length}")
      val (label,data) = if(splits.length==6){
