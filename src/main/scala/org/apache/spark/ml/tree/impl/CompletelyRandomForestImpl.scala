...
//          logWarning(s"delete all checkpoints failed. Error reason: ${e.getMessage}")
  ...
  private[tree] def findRandomSplits(input: RDD[BaggedPoint[TreePoint]],
    ...
    timer.start("findBestSplits - chooseSplits - getNodeToFeatures")
    val nodeToFeatures = getNodeToFeatures(treeToNodeToIndexInfo)
    timer.stop("findBestSplits - chooseSplits - getNodeToFeatures")
    timer.start("findBestSplits - chooseSplits - broadcast NodeToFeatures")
    val nodeToFeaturesBc = input.sparkContext.broadcast(nodeToFeatures)
    timer.stop("findBestSplits - chooseSplits - broadcast NodeToFeatures")
    timer.start("findBestSplits - chooseSplits - get partitionAggregates")
    val partitionAggregates: RDD[(Int, TreeStatsAggregator)] = if (nodeIdCache.nonEmpty) {
      input.zip(nodeIdCache.get.nodeIdsForInstances).mapPartitions { points =>
        ...
        val nodeStatsAggregators = Array.tabulate(numNodes) { nodeIndex =>
          ...
          new TreeStatsAggregator(metadata, featuresForNode)
        }
...
//            ", impurity = " + stats.rightImpurity)
  ...
  private[tree] def binsToRandomSplit(binAggregates: TreeStatsAggregator,
    ...
    val (randomSplit, randomSplitStats) =
      if (featureSplitsAndImpurityStats.isEmpty) {
  ...
  protected[tree] def findSplits(input: RDD[LabeledPoint],
    ...
    val sampledInput = if (continuousFeatures.nonEmpty) {
      ...
      val requiredSamples = math.max(metadata.maxBins * metadata.maxBins, 1.0 * metadata.numExamples)
    ...
    findSplitsByDicing(sampledInput, metadata, continuousFeatures,
    ...
    val continuousSplits: scala.collection.Map[Int, Array[Split]] = {
      ...
      input
        ...
        .map { case (idx, samples) =>
          val thresholds = findSplitsForContinuousFeature(samples, metadata, idx, seed)
  ...
  private[tree] def findSplitsForContinuousFeature(featureSamples: Iterable[Double],
    ...
    val splits = if (featureSamples.isEmpty) {
      ...
      val valueCountMap = featureSamples.foldLeft(Map.empty[Double, Int]) {
        case (m, x) =>
      ...
      if (possibleSplits == 0) {
        ...
        val possible = (1 to possibleSplits)
          .map(index => (valueCounts(index - 1)._1 + valueCounts(index)._1) / 2.0)
          .toIterator
        SamplingUtils.reservoirSampleAndCount(possible, numSplits, seed)._1.sorted
      }
    }
    splits
  }
  ...
  (Map[Int, Array[LearningNode]], Map[Int, Map[Int, NodeIndexInfo]]) = {
    ...
    while (nodeStack.nonEmpty && (memUsage < maxMemoryUsage || memUsage == 0)) {
      ...
      val nodeMemUsage = CompletelyRandomForestImpl
